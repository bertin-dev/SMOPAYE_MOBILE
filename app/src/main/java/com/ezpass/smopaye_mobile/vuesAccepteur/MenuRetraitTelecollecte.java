...
public class MenuRetraitTelecollecte extends AppCompatActivity
    ...
    private static final String TAG = "MenuRetraitTelecollecte";
    ...
    @BindView(R.id.lntelecollecteAccepteur)
    LinearLayout lntelecollecteAccepteur;

    @BindView(R.id.idCarteCacheTelecollecte)
    EditText idCarteCacheTelecollecte;
                            ...
                            if(!toHexString(uid).equals("")){
                                ...
                                for ( ScoreData score : scores ) {
                                    if(score.getMontant()==0){
                                        ...
                                        montant = score.getMontant();
                                    }
                                }

                                telecollecteInSmopayeServer(idCarteCacheTelecollecte.getText().toString().trim(), String.valueOf(montant), getSerialNumber().trim(), temp_number);
    ...
    private void telecollecteInSmopayeServer(String id_card, String montant, String serial_number, String telephone) {
        ...
        call = service.telecollecte(Integer.parseInt(montant), id_card, serial_number);
        call.enqueue(new Callback<AccessToken>() {
                ...
                if(response.isSuccessful()){
                   ...
                   /* if(response.code() == 422){
                    ...
                    if(response.code() == 401){
                        ...
                        Toast.makeText(MenuRetraitTelecollecte.this, apiError.getMessage(), Toast.LENGTH_SHORT).show();
                    } else {
                        errorResponse(id_card, "");
                    }
                }
            ...
            public void onFailure(Call<AccessToken> call, Throwable t) {
                ...
                if(!(activeInfo != null && activeInfo.isConnected())){
                    ...
                    lntelecollecteAccepteur.setVisibility(View.GONE);
    ...
    private void successResponse(String id_card, String response) {
        ...
        int finalMontantAct = montantAct;
                ...
                if(dataSnapshot.exists()){
                    for (DataSnapshot userSnapshot : dataSnapshot.getChildren()) {
                        ...
                        if (user.getId_carte().equals(id_carte_sm)) {
                            RemoteNotification(user.getId(), user.getPrenom(), getString(R.string.telecollecte) + " ", getString(R.string.compteCredite)+ " " + finalMontantAct + " FCFA", "success");
                        } else {
                            Toast.makeText(MenuRetraitTelecollecte.this, getString(R.string.numeroInexistant), Toast.LENGTH_SHORT).show();
                        }
                    }
                }
                else{
        ...
        LocalNotification(getString(R.string.telecollecte), response);

        ////////////////////INITIALISATION DE LA BASE DE DONNEES LOCALE/////////////////////////
        dbHandler = new DbHandler(getApplicationContext());
        aujourdhui = new Date();
        shortDateFormat = DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT);
        dbHandler.insertUserDetails(getString(R.string.telecollecte), response, "0", R.drawable.ic_notifications_black_48dp, shortDateFormat.format(aujourdhui));
    ...
    private void errorResponse(String id_card, String response){
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if(dataSnapshot.exists()){
                    for (DataSnapshot userSnapshot : dataSnapshot.getChildren()) {
                        User user = userSnapshot.getValue(User.class);
                        if (user.getId_carte().equals(id_carte_sm)) {
                            RemoteNotification(user.getId(), user.getPrenom(), getString(R.string.telecollecte), response, "error");
        ...
        aujourdhui = new Date();
        shortDateFormat = DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT);
        dbHandler.insertUserDetails(getString(R.string.telecollecte), response, "0", R.drawable.ic_notifications_red_48dp, shortDateFormat.format(aujourdhui));
    ...
    private void showSnackBar(boolean isConnected) {
        ...
        if(isConnected){
            ...
            snackbar = Snackbar.make(findViewById(R.id.menuRetraitTelecollecte), message, Snackbar.LENGTH_LONG);
            ...
            snackbar = Snackbar.make(findViewById(R.id.menuRetraitTelecollecte), message, Snackbar.LENGTH_INDEFINITE);
            view = snackbar.getView();
            TextView textView = view.findViewById(android.support.design.R.id.snackbar_text);
            textView.setTextColor(color);
            textView.setGravity(Gravity.CENTER);
