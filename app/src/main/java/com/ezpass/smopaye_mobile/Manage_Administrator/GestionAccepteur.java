...
public class GestionAccepteur extends AppCompatActivity {
    ...
    private static final String TAG = "GestionAccepteur";
    ...
    private int nbrTotalAccepteurs;
    private int nbrTotalAccepteurActif;
    private int nbrTotalAccepteursInactif;
                    ...
                    if(responseUser.isSuccess()){
                        ...
                        if(usersLists.isEmpty()){
                            ...
                            for(int i=0; i<usersLists.size();i++){
                                if(usersLists.get(i).getUser().getRole().getName().toLowerCase().trim().equalsIgnoreCase("accepteur")){
                                    result.add(usersLists.get(i));
                                    Log.w(TAG, "LISTE ACCEPTEURS: " + usersLists.get(i));
                                }
                            }

                            emptyListUsers.setVisibility(View.GONE);
                            adapter = new AllUsersAdapter(GestionAccepteur.this, result, result);
                            recyclerView.setAdapter(adapter);
                        }
                    } else{
                        Toast.makeText(GestionAccepteur.this, responseUser.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                }
                else{
                    tokenManager.deleteToken();
                    startActivity(new Intent(GestionAccepteur.this, Login.class));
                    finish();
                }
            ...
            public void onResponse(Call<HomeUsersList> call, Response<HomeUsersList> response) {
                ...
                if(response.isSuccessful()){
                    ...
                    if(responseTotalUser.isSuccess()){
                        ...
                        nbrTotalAccepteurs = 0;
                        nbrTotalAccepteurActif = 0;
                        nbrTotalAccepteursInactif = 0;

                        if(usersTotalLists.isEmpty()){
                            Toast.makeText(GestionAccepteur.this, "Liste des Accepteurs Vide", Toast.LENGTH_SHORT).show();
                        } else{

                            for(int i = 0; i<usersTotalLists.size(); i++){

                                if(usersTotalLists.get(i).getUser().getRole().getName().toLowerCase().equalsIgnoreCase("accepteur")){
                                    nbrTotalAccepteurs++;
                                    if(usersTotalLists.get(i).getUser().getState().toLowerCase().equalsIgnoreCase("activer")){
                                        nbrTotalAccepteurActif++;
                                    }else {
                                        nbrTotalAccepteursInactif++;
                                    }
                                }
                            }

                            totalAccepteur.setText(String.valueOf(nbrTotalAccepteurs));
                            totalAccepteurActifs.setText(String.valueOf(nbrTotalAccepteurActif));
                            totalAccepteurInactifs.setText(String.valueOf(nbrTotalAccepteursInactif));
