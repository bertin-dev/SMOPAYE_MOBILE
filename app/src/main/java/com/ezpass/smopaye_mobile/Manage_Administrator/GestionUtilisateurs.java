...
public class GestionUtilisateurs extends AppCompatActivity {

    private static final String TAG = "GestionUtilisateurs";
    ...
    private int nbrTotalUtilisateurs;
    private int nbrTotalUtilisateursActif;
    private int nbrTotalUtilisateursInactif;
    private HomeUsersList responseTotalUser;
    private List<AllParticuliersList> usersTotalLists;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle(getString(R.string.GesUser));
                    ...
                    if(responseUser.isSuccess()){
                        ...
                        if(usersLists.isEmpty()){
                            ...
                            for(int i=0; i<usersLists.size();i++){
                                if(usersLists.get(i).getUser().getRole().getName().toLowerCase().trim().equalsIgnoreCase("utilisateur")){
                                    result.add(usersLists.get(i));
                                    Log.w(TAG, "LISTE UTILISATEURS: " + usersLists.get(i));
                                }
                            }

                            emptyListUsers.setVisibility(View.GONE);
                            adapter = new AllUsersAdapter(GestionUtilisateurs.this, result, result);
                            recyclerView.setAdapter(adapter);
                        }
                    } else{
                        Toast.makeText(GestionUtilisateurs.this, responseUser.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                }
                else{
                    tokenManager.deleteToken();
                    startActivity(new Intent(GestionUtilisateurs.this, Login.class));
                    finish();
                }
    ...
    private void showCompteurTotalUsers() {
        ...
        call.enqueue(new Callback<HomeUsersList>() {
            ...
            public void onResponse(Call<HomeUsersList> call, Response<HomeUsersList> response) {
                ...
                if(response.isSuccessful()){
                    assert response.body() != null;
                    responseTotalUser = response.body();
                    if(responseTotalUser.isSuccess()){
                        //Toast.makeText(ListAllCardSaved.this, myResponse.getMessage(), Toast.LENGTH_SHORT).show();
                        usersTotalLists = responseTotalUser.getData();

                        nbrTotalUtilisateurs = 0;
                        nbrTotalUtilisateursActif = 0;
                        nbrTotalUtilisateursInactif = 0;

                        if(usersTotalLists.isEmpty()){
                            Toast.makeText(GestionUtilisateurs.this, "Liste des Utilisateurs Vide", Toast.LENGTH_SHORT).show();
                        } else{

                            for(int i = 0; i<usersTotalLists.size(); i++){

                                if(usersTotalLists.get(i).getUser().getRole().getName().toLowerCase().equalsIgnoreCase("utilisateur")){
                                    nbrTotalUtilisateurs++;
                                    if(usersTotalLists.get(i).getUser().getState().toLowerCase().equalsIgnoreCase("activer")){
                                        nbrTotalUtilisateursActif++;
                                    }else {
                                        nbrTotalUtilisateursInactif++;
                                    }
                                }
                            }

                            totalAccepteur.setText(String.valueOf(nbrTotalUtilisateurs));
                            totalAccepteurActifs.setText(String.valueOf(nbrTotalUtilisateursActif));
                            totalAccepteurInactifs.setText(String.valueOf(nbrTotalUtilisateursInactif));
